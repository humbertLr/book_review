Value Proposition Canvas
start with the customer

# Module 1: Introduign the tools to formulate fit
## 1.1 Module Overview
## 1.2 The why: 
The pain to be avoided
avoid the pains in doing that job
## 1.3 The value proposition canvas 
### The job to be done
### The pain to be avoided
### The gains to be captured
including socail gains like kinship

# Module 2: Business Model Canvas, BMC
## Parts (9 building blocks)
### Customer Part
Who are your customers, how do you find them, how's your relationship?
Customer Segments
Cutomer Relationships
Channels
### Value Proposition
What can you provide to the customers, why will they choose you?
### Key Part
Who's your friend when doing this, what do you do and what resources you have?
Key Partnership
Key Activities
Key Resource
### Cost Structure
anything you will spend, including time, money and so on
### Revenue Streams
how's your gain
## Blackberry案例
### 事件层
 1). 竞争维度的转变：iphone的出现让竞争从硬件+企业安全转向了软件生态+操作系统+用户体验；RIM认识到了这一点，但在行动层面没有执行。
 2). 战略信誉的破产：iphone killer storm的失败，使得战略盟友转向
 3). 技术转型慢：操作系统的转变决策慢，时间拖后；在技术行业，晚到的正确也会成为错误
 4). Playbook称为资源黑洞
 5). 内部战略方向的分歧：关于BMS是否跨平台的问题，高层产生了分歧，同时也失去了机会
 6). 产品顺序错误：先发布Z10触摸屏，既没有赢得新客户，也失去了老客户
### 结构级失败原因
 1). 双ceo结构在转型期失效
 2). 工程师文化的傲慢：没有捕捉到用户真正想要的，把工程理性达成了用户理性
 3). 内部政治+中层过滤
 4). 战略资源分散：转型期的道路太杂乱
### 可迁移启示
 1).核心优势不能轻易放弃：系统失去了唯一性，就会进入规模竞争
 2). 把握技术窗口
 3). 敢于自己颠覆自己，不要等着别人颠覆
### 巅峰期Blackberry的BMC
 1). Customer Segments
 **核心客户**
 企业客户（政府、金融机构、大型公司）
 企业 IT / CIO（真正的决策者)
 
 **次级客户**
 商务人士
 新兴市场用户（BBM + PIN）
 真正的客户不是个人用户
 客户群体逐渐向个人消费者转移
 2). Value propositions
 安全
 高效
 稳定
 BBM
 个人用户更在意APP，流畅和体验
 3). Channels
 无自营销售
 依赖运营商
 企业统一采购
 合作伙伴转向，运营商不再偏爱
 4). Customer Relationships
 长期签约
 企业支持
 CIO信任关系强
 高粘性+低流动
 用户关系崩溃
 5). Revenue Streams
 服务费很高
 服务费模式被否定
 6). Key Resources
 NOC
 安全协议
 BBM网络
 NOC变成成本负担，专有系统变成生态劣势
 和运营商的谈判能力
 7). Key activities
 硬件优化
 网络压缩
 活动对用户已经不再重要
 8). Key Partership
 运营商
 企业，政府
 运营商转向，合作关系变成博弈
 9). Cost structures
 研发，集中在硬件
 销售团队
 运营网络
 收入下降，成本无法缩减
## 用BMC拆解人生
### 解释
本质：在长期市场内，出售个人能持续交付的价值
1). Customer Segments
你在为谁创造价值：公司、市场、合作者、未来的自己
谁在决定你的价值，你是否依赖单一客户
**避免客户单一化**
2). Value Propositions
把自己当作一个公司或商品，你的卖点是什么，别人为什么愿意接受？
找到自己的稀缺价值
3). Channels
你是如何被看见的？渠道是否足够丰富？
4). Customer Relationships
你和客户的关系如何，是一次性交付还是长期合作，你是否是不可替代的？
你是在被管理还是在被信任？
5). Revenue Streams
你是如何获得回报的？
要注意通过出卖时间来换取回报，以及延迟回报
6). Key Resources
你最核心的资源是什么？
要能够反复调用，不单单是拥有它
7). Key Activities
你每天主要在做什么？
增长性活动有助于创造未来
8). Key Parterships
你在和谁共同成长？
9). Cost structure
你的人生成本是什么？时间、注意力、金钱、情绪
你的成本是否在为价值服务
**低风险模型：多客户 → 稀缺判断 → 主动渠道 → 复利回报 → 可放大资源**
### Key Activities重构
1). 结构化拆解 (输出结构，不追求结论漂亮)
1. 这是什么系统？
2. 它的目标函数是什么？
3. 核心约束在哪里？
4. 谁在付出成本？谁在拿好处？
5. 哪个变量一变，系统就会崩？
拆解内容
- 公司
- 职业路径
- 商业模式
- 社会机制
2). 写出拆解
写作不是为了表达，而是为了压实思考
3). 模型复盘
问三个问题：
1. 我这周反复用了哪个结构？
2. 哪个问题我越拆越快？
3. 有没有一个模型值得长期保留？
命名模型进行复用
4). 集中存档